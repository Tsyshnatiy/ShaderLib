...
void main() {
    ...
    vec2 velocity = vec2(u_time);
    
    float snoise_mult = 0.234;
    float snoise_add = 0.306;
    float flow = snoise(st + velocity) * snoise_mult + snoise_add;
    ...
    color = smoothstep(0.7, 0.79, color);
